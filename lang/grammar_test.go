...
// TestGrammarLoads confirms the grammar textproto produced by
// cmd/gengrammar round-trips through prototext and contains the
// expected top-level rules.
func TestGrammarLoads(t *testing.T) {
	blob, err := os.ReadFile("sqlite-grammar.textproto")
	if err != nil {
		t.Fatalf("read: %v", err)
	}
